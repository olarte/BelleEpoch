// engine/index.js — Clearing engine core + epoch timer
...
async function runEpoch() {
  ...
  settleEpoch(epochId).then(async (settlements) => {
    ...
    // Belle earns as provider: clearingPrice × slotsFilled
    // Route 80% to Bankr wallet to fund Venice inference
    if (result.slotsFilled > 0 && bankr.isInitialized()) {
      ...
      // Track Belle's wins specifically
      if (belleWon) {
    ...
    // Route 1.5% protocol fee through Uniswap v4 pool on Base
    ...
    // ─── Margin logging ────────────────────────────────────────────────
    // earned = provider revenue (clearingPrice × slotsFilled)
    // spent = Belle's bid cost (if she won) + estimated Venice inference cost
...
// Start the engine when run directly
if (require.main === module) {
  console.log('Belle Epoch — Clearing Engine');
  console.log(`Capacity: ${CAPACITY} slots | Epoch: ${EPOCH_DURATION}ms`);
  console.log('---');
  setInterval(runEpoch, EPOCH_DURATION);
}
